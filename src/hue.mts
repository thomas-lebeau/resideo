 ...
 */
async function getHueLightData(silent: boolean = false): Promise<HueResponse> {
  try {
 ...
 */
export async function collectHueLightData(silent: boolean = false): Promise<void> {
  try {
    if (!silent) {
    ...
    // Fetch light data
    const rawData = await getHueLightData(silent);
    if (!silent) {
